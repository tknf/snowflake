...
Create the generator **once at module scope**, not per-request — this lets an isolate that gets
reused across requests keep its state (and, for monotonic edge mode below, its ordering
guarantees) instead of paying setup cost on every invocation:

```typescript
import { createSnowflake } from '@tknf/snowflake';

// Create once at module scope, not per-request
const generateId = createSnowflake({ mode: 'edge' });

export default {
  async fetch(request): Promise<Response> {
    return new Response(generateId());
  },
...
#### High-throughput single context: monotonic edge mode

If a single isolate needs to generate many IDs in quick succession (e.g. a batch job or a
hot request path), add `monotonic: true`:

```typescript
const generateId = createSnowflake({ mode: 'edge', monotonic: true });
```

Within the same isolate, IDs are guaranteed to be strictly unique and sorted in generation
order, even when produced in the same millisecond. See
[Monotonic edge mode](#monotonic-edge-mode) for the full guarantees and behavior when the
22-bit entropy space is exhausted.

#### Durable Objects

A Durable Object instance processes requests serially (one at a time, single-threaded), so
the collision risk that motivates edge mode's entropy-based ID doesn't apply within a single
DO — `default` mode is safe to use there. Derive a stable `workerId` from the DO's own ID so
IDs generated by the same instance share a consistent worker identity:

```typescript
import { DurableObject } from 'cloudflare:workers';
import { createSnowflake } from '@tknf/snowflake';

export class IdGenerator extends DurableObject {
  // Derive a stable workerId from the DO's own ID
  #generate = createSnowflake({
    workerId: Number(BigInt('0x' + this.ctx.id.toString().slice(0, 8)) % 32n),
  });
...
> `workerId` only has 32 (0-31) possible values, so with more than 32 concurrently active DO
> instances, different instances can derive the same `workerId` and collide. If you expect
> more than 32 instances, use `mode: 'edge'` instead.

#### Service Worker

See [`examples/worker`](examples/worker) for a runnable Service Worker example.
